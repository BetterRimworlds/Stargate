...
public class BlueprintSpawner
{
    ...
    {
        if (!TryReserveRect(def, pos, rot, interior, out _))
        {
            Log.Warning($"BetterRimworlds.Stargate: Secret Lab failed to place {def?.defName ?? "null"} at {pos} rot={rot}.");
            return null;
        }
    ...
    {
        if (def == null || !pos.IsValid || map == null) return;
