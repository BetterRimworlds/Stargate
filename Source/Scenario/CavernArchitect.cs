...
public static class CavernArchitect
{
    ...
    // Internal state for post-generation queries
    private static HashSet<IntVec3> _cavernCells;
        ...
        IEnumerable<CellRect> exclusionRects = null)
    {
        ...
        try
        {
            ...
            // Store for queries
            _cavernCells = new HashSet<IntVec3>(cavernCells);
        }
        finally
        {
            Rand.PopState();
    ...
    /// Returns true if the cell was generated as cavern floor.
    /// Only valid after GenerateCavernSystem() has been called.
    public static bool IsCavernCell(IntVec3 cell)
    {
        return _cavernCells?.Contains(cell) ?? false;
    }

    /// Returns all cavern floor cells from the most recent generation.
    public static IEnumerable<IntVec3> GetCavernCells()
    {
        return _cavernCells ?? Enumerable.Empty<IntVec3>();
    }
        ...
        HashSet<IntVec3> waterCells)
    {
        ...
        while (frontier.Count > 0 && waterCells.Count < targetSize)
        {
    ...
    private static void PlantMushrooms(Map map, List<IntVec3> cavernCells, float density, float soilRatio)
    {
        ...
        ThingDef primaryRock = DefDatabase<ThingDef>.GetNamed(RockTypes[Rand.Range(0, RockTypes.Length)]);
        TerrainDef cavernFloor = DetermineCavernTerrain(primaryRock, soilRatio);
